...
if (!empty($_POST['fecha_inicio'])) {
    if ($rl->updateFechaInicio($_POST['fecha_inicio'], $_POST['id_licencia'])) {
        $actualizacionExitosa = true;
    }
} 

if (!empty($_POST['fecha_fin'])) {
    if ($rl->updateFechaFin($_POST['fecha_fin'], $_POST['id_licencia'])) {
        $actualizacionExitosa = true;
    }
} 

if (!empty($_POST['estado'])) {
    if ($rl->updateEstado($_POST['estado'], $_POST['id_licencia'])) {
        $actualizacionExitosa = true;
    }
} 

if (!empty($_POST['id_tipo_licencia'])) {
    if ($rl->updateTipoLicencia($_POST['id_tipo_licencia'], $_POST['id_licencia'])) {
        $actualizacionExitosa = true;
    }
}
